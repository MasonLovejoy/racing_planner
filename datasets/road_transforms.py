...
class RandomLateralShift:
    def __init__(self, shift_range: float = 0.3, p: float = 0.5):
    ...
    def __call__(self, sample:dict):
        if np.random.rand() < self.p:
            ...
            shift = np.array([0, shift_y], dtype=np.float32)

            if 'track_left' in sample:
                sample['track_left'] = sample['track_left'].copy()
                sample['track_left'] = sample['track_left'] + shift

            if 'track_right' in sample:
                sample['track_right'] = sample['track_right'].copy()
                sample['track_right'] = sample['track_right'] + shift

            if 'waypoints' in sample:
                sample['waypoints'] = sample['waypoints'].copy()
                sample['waypoints'] = sample['waypoints'] + shift
        return sample
        
class RandomLateralNoise:
    def __init__(self, long_std: float = 0.01, lat_std: float = 0.03, p: float = 0.5):
    ...
    def __call__(self, sample:dict):
        if np.random.rand() < self.p:
            if 'track_left' in sample:
                sample['track_left'] = sample['track_left'].copy()
                noise = np.random.normal(loc=0.0, scale=[self.long_std, self.lat_std], size=sample['track_left'].shape).astype(np.float32)
                sample['track_left'] = sample['track_left'] + noise

            if 'track_right' in sample:
                sample['track_right'] = sample['track_right'].copy()
                noise = np.random.normal(loc=0.0, scale=[self.long_std, self.lat_std], size=sample['track_right'].shape).astype(np.float32)
                sample['track_right'] = sample['track_right'] + noise
    ...
    def __call__(self, sample: dict):
        if np.random.rand() < self.p:
            ...
            if 'track_left' in sample and 'track_right' in sample:
                track_left = sample['track_left'].copy()
                track_right = sample['track_right'].copy()

                track_left[:, 1] *= -1
                track_right[:, 1] *= -1

                sample['track_left'] = track_right
                sample['track_right'] = track_left
